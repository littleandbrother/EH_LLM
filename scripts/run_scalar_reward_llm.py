...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run VEHBench scalar-reward-only LLM ablation.")
    parser.add_argument(
        "--tasks-file",
    ...
    parser.add_argument(
        "--output-root",
        default="artifacts/runs/scalar_reward_llm",
...
def main() -> None:
    ...
    run_id = f"vehbench_scalar_reward_llm_{task_slug}_{split_slug}_{stamp}"
    output_dir = Path(args.output_root) / run_id
    result = run_benchmark(
        tasks=tasks,
        solver_names=["scalar_reward_llm"],
        seed=args.seed,
