...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run VEHBench verifier-guided LLM baseline.")
    parser.add_argument(
        "--tasks-file",
    ...
    parser.add_argument(
        "--output-root",
        default="artifacts/runs/verifier_guided_llm",
...
def main() -> None:
    ...
    run_id = f"vehbench_verifier_guided_llm_{task_slug}_{split_slug}_{stamp}"
    output_dir = Path(args.output_root) / run_id
    result = run_benchmark(
        tasks=tasks,
        solver_names=["verifier_guided_llm"],
        seed=args.seed,
